...
namespace BookSelection.WebApp
   ...
   public static class Helper
      ...
      static private Regex _fieldParser = new Regex(@"\[(?<Lookup>\w+)\]", RegexOptions.IgnoreCase);
		...
		/// </summary>
		public static void PostError(Exception exp, HttpContext context, bool showStack = false)
			{
			PostError(exp, context.Request.RequestContext.HttpContext, showStack);
			}

		public static void PostError(Exception exp, HttpContextBase context, bool showStack = false)
			{
      ...
      /// <returns></returns>
		public static string PostRequest(out Uri responseUri, HttpRequest inboundRequest, string targetPath, IDictionary<string, string> fieldMap = null)
			{
			return PostRequest(out responseUri, inboundRequest.RequestContext.HttpContext.Request, targetPath, fieldMap);
			}

		public static string PostRequest(out Uri responseUri, HttpRequestBase inboundRequest, string targetPath, IDictionary<string, string> fieldMap = null)
			{
